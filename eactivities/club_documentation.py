...
class ClubDocumentation(object):
    def __init__(self, club):
        self.club = club

    def inventory(self):
        return ClubInventory(self)

    def risk_assessment(self):
        return ClubRiskAssessment(self)

    def key_lists(self):
        return ClubKeyLists(self)


class ClubRecordDocument(object):
    document_name = None
    column_map = {}
    ...
    def eactivities(self):
        return self.documentation.club.eactivities

    def optimus_prime(self):
        document_soup, _ = self.eactivities.load_and_start(
            '/admin/csp/documents/{}'.format(self.documentation.club.id)
        )
...
class ClubInventory(ClubRecordDocument):
...
class ClubRiskAssessment(ClubRecordDocument):
...
class ClubKeyLists(ClubRecordDocument):
    document_name = 'Key List'

    def list(self):
        enc_tab, document_soup = self.optimus_prime()
        ...
        for key_list_tab_id in key_list_tab_ids:
            ...
            # process it
            out.append(self.handle_item(key_list_tab))
